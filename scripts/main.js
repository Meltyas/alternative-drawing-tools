...
class PrecisionDrawLayer extends foundry.canvas.layers.InteractionLayer {
  ...
  /** Id of the stroke selected with the select tool. */
  #selectedId = null;
  ...
  setTool(name) {
    ...
    if (name !== "select") this.#setSelected(null);
    this.#drawEraserRing();
  ...
  async #cycleColor(direction) {
    ...
    let idx = SWATCHES.findIndex((c) => c.toLowerCase() === current);
    idx = idx === -1
      ? (direction > 0 ? 0 : SWATCHES.length - 1)
      : (idx + direction + SWATCHES.length) % SWATCHES.length;
    await setSetting(SETTINGS.color, SWATCHES[idx]);
    this.#afterStyleChange();
  }
  ...
  _onClickLeft(event) {
    ...
    switch (this.#tool) {
      case "eraser":
        ...
        break;
      case "select":
        this.#setSelected(this.#hitTest(origin)?.id ?? null);
        break;
      case "text":
  ...
  _onDragLeftDrop() {
    ...
    if (this.#dragState?.type === "move") {
      const { id, start, delta } = this.#dragState;
      this.#dragState = null;
      if (delta.x || delta.y) {
        this.#updateStrokes([{ _id: id, x: Math.round(start.x + delta.x), y: Math.round(start.y + delta.y) }])
          .then(() => this.#drawSelection());
      }
      return;
    }
  ...
  /** Selection box; with `delta`, previews the move. */
  #drawSelection(delta = { x: 0, y: 0 }) {
    const g = this.selectionG;
    g.clear();
    if (!this.#selectedId) return;
    const doc = canvas.scene.drawings.get(this.#selectedId);
    if (!doc) {
  ...
  /** Redraws the selection if the document changed/disappeared (global hook). */
  _onDocChange(doc) {
    if (doc.id === this.#selectedId) this.#drawSelection();
  }
  ...
  /**
   * Reorders the selected stroke within its elevation: "front" | "back" |
   * "up" | "down". Normalizes the sort of all module strokes so the
   * resulting order is exact.
   */
  async reorderSelected(mode) {
    const doc = this.#selectedId ? canvas.scene.drawings.get(this.#selectedId) : null;
    if (!doc) {
      ui.notifications.info("Drawing Tools: select a stroke first.");
      return;
    }
    const docs = this.#strokeDrawings()
      .filter((d) => d.elevation === doc.elevation)
      .sort((a, b) => (a.sort - b.sort) || a.id.localeCompare(b.id));
    const order = docs.map((d) => d.id).filter((id) => id !== doc.id);
    const idx = docs.findIndex((d) => d.id === doc.id);
    const insertAt = {
      back: 0,
      down: Math.max(0, idx - 1),
      up: Math.min(order.length, idx + 1),
      front: order.length
    }[mode] ?? order.length;
    order.splice(insertAt, 0, doc.id);
  ...
  async clearAll() {
    ...
    const ok = await foundry.applications.api.DialogV2.confirm({
      window: { title: "Alternative Drawing Tools" },
      content: `<p>Delete <strong>${ids.length}</strong> stroke(s) from the scene?</p>`
    });
    if (!ok) return;
...
class DrawToolbar {
  ...
  static #build() {
    ...
    el.innerHTML = `
      ...
      <button type="button" data-action="clear" title="Delete all strokes in the scene"><i class="fa-regular fa-trash-can"></i></button>
...
class DrawPalette {
  ...
  static #build() {
    ...
    const strokeSwatches = SWATCHES.map((c) =>
    ...
    el.addEventListener("click", async (event) => {
      const btn = event.target.closest("button");
      if (!btn) return;
      if (btn.dataset.color) await setSetting(SETTINGS.color, btn.dataset.color);
      else if (btn.dataset.fill !== undefined) await setSetting(SETTINGS.fill, btn.dataset.fill);
      else if (btn.dataset.width) await setSetting(SETTINGS.width, Number(btn.dataset.width));
        ...
        canvas[LAYER_NAME]?.reorderSelected(btn.dataset.layer);
        return;
      } else return;
      this.#sync();
    });
    ...
    el.querySelector("#gpd-alpha").addEventListener("input", async (ev) => {
      await setSetting(SETTINGS.alpha, Number(ev.target.value) / 100);
    });
